...
QSize ResultItemDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    Q_UNUSED(option)
    Q_UNUSED(index)
    return {-1, PADDING_S + BUTTON_SIZE + PADDING_S};
}
                                           ...
                                           const int &hoveredActionIndex, const bool &isSelected, const bool &isHovered)
{
    ...
    {
        const int buttonX = rect.right() - (actionCount - actionIndex) * (BUTTON_SIZE + PADDING_S);
        const QRect buttonRect(buttonX, rect.top() + PADDING_S, BUTTON_SIZE, BUTTON_SIZE);
...
{
    return {
        itemRect.left() + PADDING_S + (BUTTON_SIZE - ICON_SIZE) / 2, //
...
{
    constexpr int leftMargin = PADDING_S + BUTTON_SIZE + PADDING_S;
    constexpr int topMargin = PADDING_S;
    return {
...
{
    const int actionsWidth = (actionsCount - 1) * (BUTTON_SIZE + PADDING_S) + PADDING_S; // The primary action is not displayed as a button.
...
int ResultItemDelegate::getActionButtonIndex(const QPoint &pos, const QRect &actionsRect, const int &actionCount)
{
    ...
    const int startX = actionsRect.left() + PADDING_S;
    const int relativeX = pos.x() - startX;
    const int relativeY = pos.y() - actionsRect.top();

    if (relativeX < 0)
        return 0;
    if (relativeY < PADDING_S || relativeY > PADDING_S + BUTTON_SIZE)
        return 0;

    const int buttonIndex = relativeX / (BUTTON_SIZE + PADDING_S);
    const int buttonOffset = relativeX % (BUTTON_SIZE + PADDING_S);
